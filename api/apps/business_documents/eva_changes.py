...
class EvaDocumentChangeService:
    ...
    def _validate_text(value: Any, field: str, *, maximum: int, required: bool = True) -> str:
        field_label = {
            "connector_id": "подключение EVA",
            "document_id": "документ EVA",
            "change_summary": "задача на доработку",
            "document_name": "название документа",
            "draft_markdown": "текст доработки",
            "refinement": "уточнение для агента",
            "eva_page_url": "ссылка на документ EVA",
        }.get(field, field)
        if not isinstance(value, str):
            raise ValidationError("INVALID_EVA_CHANGE", f"Поле «{field_label}» должно содержать текст.", {"field": field})
        normalized = value.strip()
        if required and not normalized:
            raise ValidationError("INVALID_EVA_CHANGE", f"Заполните поле «{field_label}».", {"field": field})
        if len(value) > maximum:
            raise ValidationError(
                "INVALID_EVA_CHANGE",
                f"Поле «{field_label}» не должно превышать {maximum:,} символов.".replace(",", " "),
    ...
    def create_change(
        ...
        raw: object,
        *,
        allow_prefilled_draft: bool = False,
    ) -> dict[str, Any]:
        ...
        if requested_draft is not None and not allow_prefilled_draft:
            raise ValidationError(
    ...
    def generate_draft(
        ...
        *,
        ...
        if not isinstance(raw, dict):
            raise ValidationError("INVALID_EVA_GENERATION", "Параметры подготовки доработки должны быть переданы в виде объекта.")
        expected = cls._expected_version(raw)
        refinement = cls._validate_text(raw.get("refinement", ""), "refinement", maximum=10_000, required=False).strip()
        change = cls._reserve_generation(tenant_id, actor_id, change_id, expected)
        reservation_version = change.state_version
        change_request = change.change_summary
        if refinement:
            change_request = f"{change_request}\n\nУточнение автора:\n{refinement}"
        if len(change_request) > 50_000:
            error = ValidationError(
                "INVALID_EVA_GENERATION",
                "Задача вместе с уточнением превышает 50 000 символов. Сократите уточнение и повторите подготовку.",
                {"maximum": 50_000},
            )
            cls._restore_after_external_failure(
                change.id,
                reservation_version,
                EvaChangeState.GENERATING_DRAFT,
                EvaChangeState.EDITING,
                error,
            )
            raise error
        ...
        try:
            ...
            generated = ai.generate_eva_change(tenant_id, change.base_markdown, change_request)
            draft_markdown = cls._validate_text(
                generated.get("draft_markdown"),
                "draft_markdown",
                maximum=_MAX_DRAFT_SIZE,
            )
            draft_markdown = _canonical_markdown(draft_markdown)
            draft_hash = _content_hash(draft_markdown)
            if draft_hash == change.base_content_hash:
                raise ValidationError(
                    "EVA_AI_DRAFT_UNCHANGED",
                    "Агент не изменил документ. Уточните, что именно нужно исправить, и повторите подготовку.",
                )
            draft_html = _markdown_to_html(draft_markdown)
        except Exception as error:
            mapped = (
                error
                if isinstance(error, BusinessDocumentError)
                else BusinessDocumentError(
                    "EVA_AI_GENERATION_FAILED",
                    "Агент не смог подготовить доработку EVA. Повторите попытку; если ошибка сохранится, обратитесь к администратору.",
                    502,
                )
            )
            cls._restore_after_external_failure(
        ...
        cls._get_change(tenant_id, actor_id, change_id)
        with BusinessDocumentEvaChange._meta.database.atomic():
            change = cls._get_change_row(tenant_id, actor_id, change_id)
            if change.workflow_state != EvaChangeState.GENERATING_DRAFT.value or change.state_version != reservation_version:
                raise ConflictError(
                    "STATE_VERSION_CONFLICT",
                    "Доработка EVA была изменена параллельно. Обновите страницу и повторите действие.",
                )
            cls._transition(
                change,
                EvaChangeState.EDITING,
            ...
            cls._record_event(
                ...
                {
                    "draft_content_hash": draft_hash,
                    "summary": str(generated.get("summary") or "")[:1000],
                },
            )
        return cls._projection(change)
    ...
    def _reserve_generation(
        ...
        with BusinessDocumentEvaChange._meta.database.atomic():
            change = cls._get_change_row(tenant_id, actor_id, change_id)
            cls._require_version(change, expected_version)
            if change.workflow_state == EvaChangeState.GENERATING_DRAFT.value:
                age_ms = current_timestamp() - int(change.update_time or change.create_time or 0)
                if age_ms < _EXTERNAL_OPERATION_TIMEOUT_MS:
                    raise ConflictError(
                        "EVA_CHANGE_BUSY",
                        "Агент уже готовит доработку EVA. Дождитесь завершения текущей подготовки.",
                        {"state": change.workflow_state, "retry_after_ms": _EXTERNAL_OPERATION_TIMEOUT_MS - max(age_ms, 0)},
                    )
                cls._transition(change, EvaChangeState.GENERATING_DRAFT, last_error=None)
                change = cls._get_change_row(tenant_id, actor_id, change_id)
                cls._record_event(change, actor_id, "EXTERNAL_OPERATION_RETRIED", {"state": change.workflow_state})
            elif change.workflow_state == EvaChangeState.EDITING.value:
                cls._transition(change, EvaChangeState.GENERATING_DRAFT, approved_at=None, last_error=None)
                change = cls._get_change_row(tenant_id, actor_id, change_id)
                cls._record_event(change, actor_id, "AI_DRAFT_REQUESTED", {"base_content_hash": change.base_content_hash})
            else:
                raise ConflictError(
                    "EVA_CHANGE_STATE_CONFLICT",
                    "Сейчас нельзя запустить подготовку: состояние доработки EVA изменилось. Обновите страницу и проверьте доступные действия.",
                    {"state": change.workflow_state},
                )
        return cls._get_change(tenant_id, actor_id, change_id)
    ...
    def _get_change(cls, tenant_id: str, actor_id: str, change_id: str) -> BusinessDocumentEvaChange:
        change = cls._get_change_row(tenant_id, actor_id, change_id)
        # The settings service owns a top-level Peewee connection context. Keep
        # that lookup outside state-transition transactions so it cannot close
        # the connection that owns the active transaction.
        cls._require_configured_connector(change.connector_id, actor_id)
        return change

    @staticmethod
    def _get_change_row(tenant_id: str, actor_id: str, change_id: str) -> BusinessDocumentEvaChange:
    ...
    def _projection(cls, change: BusinessDocumentEvaChange) -> dict[str, Any]:
        ...
        if change.workflow_state == EvaChangeState.EDITING.value:
            allowed_actions.append("GENERATE_DRAFT")
            if diff["changed"]:
        ...
        if change.workflow_state in {
            EvaChangeState.GENERATING_DRAFT.value,
            EvaChangeState.PREPARING_EVA_DRAFT.value,
            EvaChangeState.PUBLISHING.value,
        }:
