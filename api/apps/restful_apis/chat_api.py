...
async def transcription():
    ...
    async def event_stream():
        ...
        try:
            ...
            close_stream = getattr(stream_iterator, "close", None) if stream_iterator is not None else None
            if callable(close_stream):
                try:
                    await asyncio.to_thread(close_stream)
                except Exception:
                    logging.getLogger(__name__).warning("Failed to close configured streaming ASR iterator", exc_info=True)
            try:
                os.remove(temp_audio_path)
            except Exception as e:
                logging.error(f"Failed to remove temp audio file: {str(e)}")
