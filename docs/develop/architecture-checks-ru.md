...
T3 control PR #4/#12/#31 последовательно интегрированы в `main`; PR #31 слит exact merge-коммитом `7843651969329f34accf537e902fa77c7720a4ff`. Первый post-integration `opened` PR #32 в run `34777290557` при пустом payload без retrigger стабильно разрешил synthetic merge `7d24b626030517461d81d45de3292474f64b6bf3` и проверил exact base/head parents, то есть primary merge-ref race исправлена. Полный run не принят: trusted fixture harness скопировал test files без protected resolver dependency и получил `6 failed, 112 passed, 118 subtests passed`; final корректно остался fail-closed, PR #32 закрыт без merge. Candidate PR #33 содержит closure fix `2eba9b2c69204da713a26b5acaaa96c6d7f64512`, cache-clean follow-up `4ae9c53a0` и parameterized-JUnit follow-up `0296ac478`: неполный source bundle отклоняется до pytest, `.pytest_cache` не меняет candidate snapshot, а exact logical case inventory принимает только корректные `name[param]` расширения и отклоняет missing/unexpected cases. Windows subset дал `132 passed, 1 skipped`; полный non-root Linux suite — `485 passed` без skip. `synchronize` run `34797814944` затем доказал, что event payload может содержать предыдущий test merge при уже актуальном stable ref. Follow-up `7b8bb6c21241453a99169fb61d4263abb72d51df` оставляет payload подписанным advisory-наблюдением: focused Python 3.13.12 matrix дала `35 passed`, полный isolated Linux Python 3.13.11 quality-suite — `487 passed` за `85.80 s`, без skip. Исправления ещё не интегрированы. Репозиторий остаётся personal-account: organization Required Workflow, merge queue, защита default branch и обязательное независимое одобрение владельца control paths не настроены и не проверены. Deploy и ruleset не выполнялись.
...
Selector не опирается только на расширение файла. Изменение registry, global configuration или контракта расширяет набор по потребителям. T3 реализует exact path/prefix/suffix selector, использует свежий T0 и базовую карту для удалённых/переименованных путей и сохраняет причину `NOT_APPLICABLE`. Protocol v1 позволяет workflow запускать selector/aggregate из comparison base и той же реализацией доказывать, что candidate policy не удалила lane/rule/fixture и не сузила coverage. Plan, candidate analysis и final aggregate выполняются в трёх разных job; финальный job на свежем runner повторно извлекает base authority и не запускает candidate lifecycle. PR event должен загружать definition из trusted base, а все job обязаны использовать один immutable candidate SHA. Trusted resolver обязан в одном bounded retry выполнить exact remote read, fetch реального объекта и повторный remote read, потребовать стабильное равенство SHA и точные event base/head parents до выпуска `candidate_sha`, а затем связать проверенный object с checkout. Event payload merge SHA валидируется и входит в signed receipt как advisory-наблюдение `ABSENT/MATCH/DIFFERENT`; live `synchronize` доказал, что payload может хранить предыдущий test merge commit, поэтому он не участвует в выборе candidate. `ABSENT`, stale, changing, malformed или недоступный ref дают fail-closed `INCOMPLETE`, а не fallback к payload, PR head или default branch. Несовпадающий parent остаётся `STALE`: отдельный head ref не доказывает, отстают refs от нового event или event уже старый. Проверка родителей только после выхода из polling недостаточна: она безопасно отклоняет stale ref, но не дожидается current identity. Merge queue проверяет собственный HEAD через отдельный `merge_group/checks_requested`. Mutable Action tag, `workflow_dispatch` на candidate ref, сохранённые checkout credentials либо `cancel-in-progress` являются POL-02 failure. Protocol v1 уже находится в `main`, но до приёмки исправленного primary trigger, ruleset, независимого approval и доступного merge-group режима остаётся `REPORT_ONLY / NOT_ENABLED`.
...
PR #31 интегрировал resolver в trusted `main`. PR #32 run `34777290557` доказал исправленный primary `opened` identity path, но одновременно выявил неполную fixture dependency materialization; поэтому он не является положительной приёмкой. PR #33 закрыл closure/cache/JUnit defects локальными Windows/Linux проверками, после чего `synchronize` run `34797814944` выявил stale event payload при уже текущем exact merge ref. Functional follow-up `7b8bb6c21241453a99169fb61d4263abb72d51df` сохраняет payload только как подписанное advisory-наблюдение и оставляет current authority у stable fetched object с exact parents; focused Python 3.13.12 matrix дала `35 passed`, полный isolated Linux Python 3.13.11 quality-suite — `487 passed in 85.80 s`, без skip. До отдельной интеграции и повторной GitHub-матрицы это только candidate evidence. Подробности записаны в [отчёте T3](t3-architecture-policy-ru.md#статус-повторного-review-control-update-2026-09-13). Механизм остаётся report-only, пока полный post-integration gate, реальная связь required check с PR/queue SHA и independent control-owner approval не закреплены repository enforcement.
...
| Candidate заменяет `pull_request_target` на `pull_request`, возвращает manual dispatch или одноимённый green job | POL-02 / required trusted run должен остаться обязательным; duplicate context не удовлетворяет gate |
| `merge_group` отсутствует либо проверяет не queue HEAD | POL-02 / merge queue не включается, T3 остаётся `INCOMPLETE` |
...
| Автор PR — единственный control owner и approval отсутствует | POL-02 / merge запрещён; self-approval не заменяет независимое review |
