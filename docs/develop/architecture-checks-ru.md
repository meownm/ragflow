...
T3 candidate прошёл base-mode положительную и отрицательную GitHub-матрицу, включая Docker OS-boundary configured Python runtime probes. Это доказывает готовность report-only механизма, но не включает enforcement: Required Workflow ruleset, защита default branch и обязательное одобрение владельца control paths пока не настроены и не проверены.
