...
T3 control PR #4/#12/#31 последовательно интегрированы в `main`; PR #31 слит exact merge-коммитом `7843651969329f34accf537e902fa77c7720a4ff`. Первый post-integration `opened` PR #32 в run `34777290557` при пустом payload без retrigger стабильно разрешил synthetic merge `7d24b626030517461d81d45de3292474f64b6bf3` и проверил exact base/head parents, то есть primary merge-ref race исправлена. Полный run не принят: trusted fixture harness скопировал test files без protected resolver dependency и получил `6 failed, 112 passed, 118 subtests passed`; final корректно остался fail-closed, PR #32 закрыт без merge. Исправление `2eba9b2c69204da713a26b5acaaa96c6d7f64512` материализует exact protected fixture closure, а неполный plan отклоняет до pytest; focused Windows subset дал `130 passed, 1 skipped`, полный non-root Linux suite — `483 passed` без skip. Исправление ещё не интегрировано. Репозиторий остаётся personal-account: organization Required Workflow, merge queue, защита default branch и обязательное независимое одобрение владельца control paths не настроены и не проверены. Deploy и ruleset не выполнялись.
