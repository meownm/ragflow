...
Workflow без label/path filters разделяет trusted plan, candidate analysis и fresh final aggregate; base-owned `pull_request_target`, `merge_group`, immutable cross-job candidate SHA, SHA-pinned Actions и отключённое credential persistence закреплены локальными positive/negative contracts. Stable merge-ref resolver выполняет exact `read/fetch/read`, проверяет fetched object, ordered event parents и payload внутри bounded retry, связывает принятый object с plan checkout и выпускает receipt, который analysis/final проверяют base-owned source. `ABSENT/STALE/CHANGING/PAYLOAD_MISMATCH` не выпускают SHA; mismatched parent остаётся `STALE` до deadline, malformed/trust failures дают ненулевой fail-closed result. Его exact fixture nodeids, включая параметризованные cases и исполняемые source mutations, входят в protected policy sources. Static producers используют isolated safe-path runner; configured Python architecture producer и его runtime children выполняются через Docker с read-only candidate/trusted/rootfs, без network/IPC, одним single-file report mount и runtime identity `65534:65534` без supplementary groups/capabilities с NNP. Отдельный unprivileged probe обязан подтвердить 12 отрицательных OS checks. Package manager, policy-fixture harness и статические analyzers не входят в эту Docker-boundary и ограничены своими base-authority/environment contracts.

PR #31 интегрировал resolver в trusted `main`. PR #32 run `34777290557` доказал исправленный primary `opened` identity path, но одновременно выявил неполную fixture dependency materialization; поэтому он не является положительной приёмкой. Локальное исправление `2eba9b2c69204da713a26b5acaaa96c6d7f64512` прошло полный non-root Linux quality-suite `483 passed` без skip, но до отдельной интеграции и повторной GitHub-матрицы остаётся только проверенным candidate. Подробности записаны в [отчёте T3](t3-architecture-policy-ru.md#статус-повторного-review-control-update-2026-09-13). Механизм остаётся report-only, пока полный post-integration gate, реальная связь required check с PR/queue SHA и independent control-owner approval не закреплены repository enforcement.
...
| Непустой event payload SHA не совпадает со стабильным ref | `PAYLOAD_MISMATCH`; failure по deadline, расхождение видно в receipt |
