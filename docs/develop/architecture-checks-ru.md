...
T3 control PR #4/#12 последовательно интегрированы в `main`; post-integration run `34706026252` подтвердил положительный base-authority путь с Python `PASS`, 19/19 runtime probes, 30/30 fixtures и 12/12 OS controls. Первичные `opened` runs `34705977822` и `34706264276` одновременно выявили пустой `pull_request.merge_commit_sha` до materialization synthetic merge ref. Первый control candidate добавил только polling и позднюю parent binding, поэтому review вернул `CHANGES_REQUIRED` по stale-ref/TOCTOU. Текущий control update для PR #31 реализует полный stateful resolver, verified plan checkout и cross-job receipt verification; автоматическая Windows/Linux матрица и local-to-remote `STABLE_CURRENT`/`SUPERSEDED` пробы прошли, открытых P1/P2 после повторного review нет. PR сохраняется Draft: публикация исправления не является integration acceptance, merge не выполняется и post-integration матрица до появления resolver в trusted `main` недоступна. Репозиторий остаётся personal-account: organization Required Workflow, merge queue, защита default branch и обязательное независимое одобрение владельца control paths не настроены и не проверены.
...
| Stable commit имеет неверное число, порядок или значения родителей | `STALE`/`EVENT_SUPERSEDED`; failure по deadline, без fallback к head/default branch |
...
| PR закрыт, конфликтен, head удалён или base изменился во время resolution | Отдельная fail-closed причина; старый event не переключается на новую identity |
