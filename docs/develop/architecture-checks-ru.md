...
T3 candidate `d6025f428dbdf0420bc14228f440a7df1b91f5ef` прошёл base-mode положительную и отрицательную GitHub-матрицу, включая Docker OS-boundary configured Python runtime probes. Enforcement-preflight затем выявил, что этот снимок не покрывал `merge_group`, допускал `cancel-in-progress` и использовал mutable Action tags. Commit `b8f7772fc165d37fba01b23b93fc869046be8767` исправляет control contract локально, но ещё не прошёл post-integration GitHub-пробы. Репозиторий остаётся personal-account: organization Required Workflow, merge queue, защита default branch и обязательное независимое одобрение владельца control paths не настроены и не проверены.
