...
T3 control PR #4/#12 последовательно интегрированы в `main`; post-integration run `34706026252` подтвердил положительный base-authority путь с Python `PASS`, 19/19 runtime probes, 30/30 fixtures и 12/12 OS controls. Первичные `opened` runs `34705977822` и `34706264276` одновременно выявили пустой `pull_request.merge_commit_sha` до materialization synthetic merge ref. Первый control candidate добавил только polling и позднюю parent binding, поэтому review вернул `CHANGES_REQUIRED` по stale-ref/TOCTOU. PR #31 на commit `35283256f97bc2d22548a0156ec3dbd5df490a3e` реализовал stateful resolver, verified plan checkout и cross-job receipt, но повторное review нашло P1: состояние `SUPERSEDED` выводилось из ref snapshot без доказательства направления обновления. Исправление опубликовано коммитом `3a10f8a92dcd2803f4d41ddb7206b96d579bc940`; Windows/Linux resolver/workflow matrix, live `STABLE_CURRENT`/`STALE` probes и полный non-root Linux quality-suite `482 passed` проходят без skip, локальный `CHANGES_REQUIRED` закрыт. Push-triggered run `34775831907` выбрал старый synthetic merge `d47e76ba…` и затем корректно заблокировал изменённые `MUST_MATCH` sources; это воспроизводит base race и не является post-integration acceptance. PR остаётся Ready и `UNSTABLE`. Merge не выполняется. Репозиторий остаётся personal-account: organization Required Workflow, merge queue, защита default branch и обязательное независимое одобрение владельца control paths не настроены и не проверены.
...
Отдельный `check_go_build_profiles.py` применяет BUILD-01 к Go-дельтам: unmapped/wrong-owner/overlap — `FAIL`, недоступный driver/toolchain, skip или отсутствие структурированного package PASS — `INCOMPLETE`. Исторические сохранённые данные этими tools не проверены. Неохваченные adapters/surfaces и итоговые TypeScript/Go policy/dead-code verdicts остаются `manual_required`/неоценёнными. T3 `check_architecture_policy.py` выбирает применимые Python architecture, runtime-graph, Go-plan и policy-fixture lanes, повторно вычисляет план при aggregation, проверяет единую identity/fingerprint, source bundle и producer/config digests и трактует отсутствие evidence как `INCOMPLETE`. Protocol v1 сверяет hashes evaluator/base/candidate policy; `compare` допускает только монотонное расширение selectors/rules/fixtures/protected sources; `materialize` извлекает intact protected producers из authority bundle; `fixtures` запускает exact base `path::nodeid` без ambient conftest/plugin injection и встраивает JUnit в identity-bound attestation. При `authority_source=base` отличие обычного protected analyzer/config/fixture/dependency закрывает lane; только candidate policy может отличаться как `BASE_FIXTURE_REVIEW`, сохраняя base evaluator/materialization и требуя ручного review. Checker и workflow остаются `MUST_MATCH`; bootstrap доказывает только candidate self-consistency. Aggregate требует authoritative/candidate plans и exact `policy-compatibility.json`; только `authority_source=base / compatibility_status=COMPATIBLE` даёт compatibility `PASS`, тогда как bootstrap остаётся `OBSERVED`. Workflow без label/path filters разделяет trusted plan, candidate analysis и fresh final aggregate; base-owned `pull_request_target`, `merge_group`, immutable cross-job candidate SHA, SHA-pinned Actions и отключённое credential persistence закреплены локальными positive/negative contracts. Локальный stable merge-ref resolver выполняет exact `read/fetch/read`, проверяет fetched object, ordered event parents и payload внутри bounded retry, связывает принятый object с plan checkout и выпускает receipt, который analysis/final проверяют base-owned source. `ABSENT/STALE/CHANGING/PAYLOAD_MISMATCH` не выпускают SHA; mismatched parent остаётся `STALE` до deadline, malformed/trust failures дают ненулевой fail-closed result. Его 36 exact fixture nodeids, включая параметризованные 88 cases и шесть исполняемых source mutations, входят в protected policy sources. Static producers используют isolated safe-path runner; configured Python architecture producer и его runtime children выполняются через Docker с read-only candidate/trusted/rootfs, без network/IPC, одним single-file report mount и runtime identity `65534:65534` без supplementary groups/capabilities с NNP. Отдельный unprivileged probe обязан подтвердить 12 отрицательных OS checks. Package manager, policy-fixture harness и статические analyzers не входят в эту Docker-boundary и ограничены своими base-authority/environment contracts. Base-mode positive/negative PR-матрица старого trigger-контракта и положительный post-integration PR run выполнены без skip; два первичных `opened` run зафиксировали fail-closed merge-ref race. Повторное review PR #31 исправило P1, полный non-root Linux quality-suite на поддерживаемом Python 3.13 завершился `482 passed` без skip, provenance имеет `unclassified=0`, а исправление опубликовано как `3a10f8a92`. Run `34775831907` ожидаемо fail-closed остановил control-source PR и одновременно вновь показал старую base race. Подробности записаны в [отчёте T3](t3-architecture-policy-ru.md#статус-повторного-review-control-update-2026-09-13). Механизм остаётся report-only, пока исправленный primary trigger, реальная связь required check с PR/queue SHA и independent control-owner approval не закреплены repository enforcement.
