...
Selector не опирается только на расширение файла. Изменение registry, global configuration или контракта расширяет набор по потребителям. T3 реализует exact path/prefix/suffix selector, использует свежий T0 и базовую карту для удалённых/переименованных путей и сохраняет причину `NOT_APPLICABLE`. Protocol v1 позволяет workflow запускать selector/aggregate из comparison base и той же реализацией доказывать, что candidate policy не удалила lane/rule/fixture и не сузила coverage. Plan, candidate analysis и final aggregate выполняются в трёх разных job; финальный job на свежем runner повторно извлекает base authority и не запускает candidate lifecycle. PR event загружает definition из trusted base, а все job используют один immutable candidate SHA. Receipt schema v3 связывает authoritative default-branch base с trusted workflow SHA; payload base/merge SHA остаются валидируемыми advisory-наблюдениями. Trusted resolver выполняет bounded exact remote read/fetch/read, требует стабильный объект и точных родителей до выпуска `candidate_sha`, затем связывает объект с checkout. `ABSENT`, stale, changing, malformed или недоступный ref дают fail-closed `INCOMPLETE`, без fallback. Merge queue в поддерживаемой topology должна проверять собственный HEAD через `merge_group/checks_requested`. Mutable Action tag, `workflow_dispatch` на candidate ref, сохранённые checkout credentials либо `cancel-in-progress` являются POL-02 failure. Машинный aggregate намеренно остаётся `REPORT_ONLY / NOT_ENABLED`; current solo-профиль дополняет его внешним no-bypass ruleset receipt и не меняет этот self-reported статус.
...
.\.venv\Scripts\python.exe -m pytest test/unit_test/business_documents test/unit_test/api/apps/business_documents test/unit_test/api/apps/restful_apis/test_business_document_api_contract.py --cov=api/apps/business_documents --cov=business_documents --cov-branch --cov-fail-under=79
...
T3 `check_architecture_policy.py` выбирает применимые Python architecture, runtime-graph, Go-plan и policy-fixture lanes, повторно вычисляет план при aggregation, проверяет единую identity/fingerprint, source bundle и producer/config digests и трактует отсутствие evidence как `INCOMPLETE`. Protocol v1 сверяет hashes evaluator/base/candidate policy; `compare` допускает только монотонное расширение selectors/rules/fixtures/protected sources; `materialize` извлекает intact protected producers из authority bundle. Для base-owned fixtures materializer обязан создать дерево из полного exact `protected_sources` closure, а не только из `fixture_paths`; отсутствующий, лишний, повторный или digest-mismatched record закрывает запуск fail-closed. Только после этого `fixtures` запускает exact base `path::nodeid` без ambient conftest/plugin injection и с отключённым pytest cache provider, чтобы test-generated `.pytest_cache` не изменял candidate snapshot, затем встраивает JUnit в identity-bound attestation. При `authority_source=base` отличие обычного protected analyzer/config/fixture/dependency закрывает lane; только candidate policy может отличаться как `BASE_FIXTURE_REVIEW`, сохраняя base evaluator/materialization и требуя ручного review. Checker и workflow остаются `MUST_MATCH`; bootstrap доказывает только candidate self-consistency. Aggregate требует authoritative/candidate plans и exact `policy-compatibility.json`; только `authority_source=base / compatibility_status=COMPATIBLE` даёт compatibility `PASS`, тогда как bootstrap остаётся `OBSERVED`.

Workflow без label/path filters разделяет trusted plan, candidate analysis и fresh final aggregate; base-owned `pull_request_target`, `merge_group`, immutable cross-job candidate SHA, SHA-pinned Actions и отключённое credential persistence закреплены локальными positive/negative contracts. Stable merge-ref resolver выполняет exact `read/fetch/read`, проверяет fetched object и ordered event parents, связывает принятый object с plan checkout и выпускает receipt, который analysis/final проверяют base-owned source. `ABSENT/STALE/CHANGING` не выпускают SHA; mismatched parent остаётся `STALE` до deadline, malformed/trust failures дают ненулевой fail-closed result. Валидный payload SHA остаётся подписанным advisory-наблюдением receipt schema v2 и не может ни выпустить candidate, ни отменить exact ref/object/parent verdict. Exact fixture nodeids, включая параметризованные cases и исполняемые source mutations, входят в protected policy sources. Static producers используют isolated safe-path runner; configured Python architecture producer и его runtime children выполняются через Docker с read-only candidate/trusted/rootfs, без network/IPC, одним single-file report mount и runtime identity `65534:65534` без supplementary groups/capabilities с NNP. Отдельный unprivileged probe обязан подтвердить 12 отрицательных OS checks. Package manager, policy-fixture harness и статические analyzers не входят в эту Docker-boundary и ограничены своими base-authority/environment contracts.

PR #31 интегрировал первый resolver; PR #33, #36 и #37 затем закрыли fixture closure/cache/JUnit, stale payload, protected dependency bundle и default-branch base authority. Current trusted `main` — `30a8123fb1f483934dc094d6cef2135ac125a3c6`. Draft PR #35 дал functional schema-v3 post-integration PASS и остаётся неслитым. Active ruleset `23306934` и diagnostic PR #38/#39 подтвердили exact-head App-bound enforcement и fail-closed negative scenarios. Подробности и ограничения записаны в [отчёте T3](t3-architecture-policy-ru.md#solo-maintainer-enforcement-receipt-2026-09-14). Механизм остаётся report-only внутри aggregate; внешний receipt позволяет только явно принятый solo-статус `VERIFIED_WITH_ACCEPTED_RISK`, не обычный `VERIFIED`.
...
В CI проверки выполняются без `--fix` и `git add`. Кандидат с изменением checker/workflow/baseline проверяется относительно политики и fixture contracts базовой ветки; нельзя доверять изменённому скрипту, самодекларированному имени analyzer или одноимённому candidate test. T3 protocol извлекает checker/capture/policy из comparison base, запрещает сужение base coverage и запускает materialized protected producers с isolated import path, а fresh final job не наследует изменяемый trusted bundle из candidate-analysis job. Метка `BASE_FIXTURE_REVIEW` и manual-review запись сами по себе не обеспечивают approval; поэтому исключение распространяется только на declarative candidate policy, тогда как checker/workflow обязаны совпадать с base. Configured Python runtime/control code изолирован от command files и supervisor-owned evidence проверенной Docker-boundary; package manager, policy-fixture harness, статические analyzers и любые ещё не настроенные runtime roots остаются вне этой гарантии. Обновление checker/workflow/protected fixtures или сужение controls требует отдельного аудированного protocol/bootstrap flow. Неизменяемость определения workflow и обязательный control-owner approval должны отдельно обеспечиваться organization Required Workflow либо доказанным эквивалентом. GitHub [не учитывает workflow, matrix и event trigger при сопоставлении required status checks](https://docs.github.com/en/enterprise-cloud@latest/repositories/configuring-branches-and-merges-in-your-repository/managing-rulesets/troubleshooting-rules#troubleshooting-required-status-checks), поэтому policy-fixtures выбирается при изменении любого `.github/workflows/*` и отклоняет duplicate или dynamic `architecture-policy` job name, но этот статический контракт не заменяет live negative probe и доверенную внешнюю привязку check. Для personal-account repository base-owned `pull_request_target` — только candidate решения: обычный required context по имени/app не считается pinned, пока live negative probe не докажет связь с актуальным PR SHA и невозможность подмены duplicate candidate check. Независимое approval невозможно при единственном collaborator/авторе и требует внешнего trusted reviewer; `CODEOWNERS` должен защищать также собственный файл.

Repository-level `default_workflow_permissions=read` полезен как least-privilege default, но не является доказательством неизменяемого control path: workflow с write-authority может запросить иные `GITHUB_TOKEN` permissions, а текущая repository policy разрешает любые Actions и не требует SHA pinning глобально. Canonical workflow компенсирует это только для собственных jobs. Поэтому финальный T3 verdict хранится во внешнем read-only enforcement receipt с live ruleset/workflow/App, check-run SHA и review evidence; самодекларируемый candidate status не переводит report-only aggregate в `VERIFIED`.

### Solo-maintainer профиль T3

Исключение из требования независимого approval допускается только по явному решению владельца и только со статусом `VERIFIED_WITH_ACCEPTED_RISK`. Для этого профиля CODEOWNERS остаётся ownership metadata, `required_approving_review_count=0`, `require_code_owner_review=false`; автоматический required gate не ослабляется. Обязательны active no-bypass ruleset, strict exact-head check, App binding, positive probe и отрицательные live-пробы failure/cancellation/stale head/control removal/user status/duplicate/skipped. Один и тот же owner не может записать саморевью как независимое доказательство.

`VERIFIED_WITH_ACCEPTED_RISK` не является альтернативным способом принять неуспешный lane. Любой применимый `FAIL` или `INCOMPLETE` блокирует; `OBSERVED`, `CLASSIFIED`, `PLANNED` и `NOT_EVALUATED` сохраняют исходный смысл. Недоступный в personal-account topology `merge_group` фиксируется как конкретный принятый риск и `NOT_AVAILABLE`, а не как `PASS` или `NOT_APPLICABLE`. Пустой bypass list ограничивает merge bypass, но не устраняет способность sole admin изменить сам ruleset; это отдельный governance risk.

External receipt должен содержать ruleset id/target/enforcement/bypass, required context/App/strictness, exact PR head и check-run identity, review count, CODEOWNERS owner, результаты положительных и отрицательных PR, cleanup и условия повторной приёмки. Изменение collaborator/admin/owner, ruleset, workflow, policy, App/context или появление organization Required Workflow/merge queue аннулирует прежний receipt до повторной проверки.
...
| На первом `opened` payload merge SHA пуст, ref появляется позже | Resolver остаётся в `ABSENT`, затем принимает только стабильный fetched commit с exact event parents; ручной retrigger не требуется |
| После `synchronize` exact ref временно указывает на предыдущий head | `STALE`, polling продолжается; поздняя отдельная parent-проверка без retry недостаточна |
| Ref изменился между первым read, fetch и вторым read | `CHANGING`, попытка отброшена; `candidate_sha` не публикуется |
| Remote output malformed, неоднозначен, указывает не на exact ref или non-commit | POL-02 / fail-closed; частичный SHA не принимается |
| Stable commit имеет неверное число, порядок или значения родителей | `STALE`; failure по deadline, без вывода направления обновления и без fallback к head/default branch |
| После `synchronize` event payload SHA остался от предыдущего test merge, а стабильный ref имеет exact текущие parents | Payload фиксируется как advisory `DIFFERENT`; candidate допускается только по `R1 == F == R2` и exact event parents, без fallback к payload |
| Проверенный объект исчез либо checkout получает иной HEAD | `CHECKOUT_UNAVAILABLE` или identity mismatch; plan/analysis/final не становятся успешными |
| PR закрыт, конфликтен, head удалён или base изменился во время resolution | `ABSENT`, `STALE` либо `UNAVAILABLE` до fail-closed deadline; старый event не переключается на новую identity |
| Candidate Git config/hook/alternate/replace/credential input влияет на resolver | POL-02 / fixture failure; resolver работает в свежем trusted temp repository с очищенным environment |
| Stateful fixtures заменены поиском строк в YAML | T3 control update не принят: tests обязаны исполнять exact resolver logic и мутации порядка/binding |
| Base fixture tree содержит только `fixture_paths`, но не protected runtime dependency | POL-02 / `INCOMPLETE`; pytest не принимается, пока exact `protected_sources` closure не материализован и не сверены digests |
| Candidate заменяет `pull_request_target` на `pull_request`, возвращает manual dispatch или одноимённый green job | POL-02 / required trusted run должен остаться обязательным; live duplicate success не удовлетворяет gate при canonical failure |
| `merge_group` отсутствует либо проверяет не queue HEAD | Обычный профиль: POL-02 / `INCOMPLETE`; solo personal-account профиль: `NOT_AVAILABLE` и явно принятый риск, никогда не PASS |
| Required workflow использует mutable Action tag, сохраняет checkout credentials или отменяет running check | BUILD-01/POL-02 / fixture failure и запрет принятия control update |
| Автор PR — единственный control owner и approval отсутствует | Обычный профиль: POL-02 / merge запрещён. Явно принятый solo-профиль: approvals `0`, CODEOWNERS только metadata, результат не выше `VERIFIED_WITH_ACCEPTED_RISK` |
