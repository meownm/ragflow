...
Дата обновления: 2026-09-14. Статус: T0 реализован для снимка из [отчёта инвентаризации](t0-provenance-report-ru.md); T1 зафиксирован как `BASELINE_COMPLETE_WITH_KNOWN_FAILURES`; T2 принят как `REPORT_ONLY_COMPLETE_WITH_CLASSIFIED_LIMITS`, а не как архитектурный/dead-code/build PASS. T3 control PR #4, #12, #31, #33, #36 и #37 последовательно слиты в `main`. PR #37 интегрирован обычным merge без bypass exact commit `30a8123fb1f483934dc094d6cef2135ac125a3c6` с родителями `[bd50920559170dbc9b042b26168062d1de777b36, 30eab490a5ded077c8928908b3580e795467d767]`; resolver schema v3 теперь берёт authoritative default-branch base из `github.sha`, требует `github.workflow_sha == github.sha` и хранит `pull_request.base.sha` только как advisory-наблюдение. Первый post-integration `edited` run PR #35 `34834573688` подтвердил эту identity-часть: trusted base и workflow SHA равны `30a8123f…`, stale payload base `312c9ce8…` записан как `DIFFERENT`. Однако GitHub не обновил сам `refs/pull/35/merge`: 42 bounded-наблюдения видели прежний merge `b384c3fc…` с родителями `[bd509205…, 7a1e0f04…]`, поэтому через 120 секунд plan завершился `STALE / PARENT_MISMATCH`, analysis не запускался, canonical final остался failure. Это обязательное отрицательное evidence, а не PASS; для приёмки schema v3 требуется fresh `synchronize` с exact родителями `[30a8123f…, current head]` и успешными plan/analysis/final. Функциональные доказательства не закрывают enforcement: успешные aggregate по-прежнему `REPORT_ONLY_COMPLETE / NOT_ENABLED`, `main` не защищена, ruleset отсутствует, repository принадлежит личному аккаунту, независимый control owner не назначен и duplicate-context/approval/control-removal probes ещё не выполнялись под реальной защитой. Поэтому T3 остаётся `IN_PROGRESS / REPORT_ONLY / NOT_ENABLED`, а T4–T7 по этой последовательности не приняты.
...
**Приёмка.** Функциональная post-integration часть DoD для receipt v2 выполнена на PR #34; повторная acceptance schema v3 после default-branch advancement ещё не завершена. Первичный `opened` при пустом payload прошёл с первой delivery; `ready_for_review`, `reopened`, одиночный и rapid `synchronize` сохранили exact identity semantics. Финальный artifact аттестует `123` fixture cases без failure/error/skip; в них выполнены controlled stale (5), changing (4), missing/disappearing (5) и non-commit/wrong-parent (5) варианты. Live rapid-проба дополнительно не выпустила candidate для устаревшего промежуточного head. Во всех успешных deliveries plan/analysis/final используют byte-identical receipt и один base/head/candidate tuple; check-run API связал canonical context с exact head, workflow, event и GitHub Actions app. Schema-v3 run `34834573688` дополнительно доказал, что stale payload base не заменяет authoritative `github.sha`, а stale merge ref не выпускает candidate. Сканирование шести architecture run logs не нашло незамаскированных GitHub token/Authorization/Actions runtime token/AWS access-key patterns. Это не заменяет оставшуюся functional schema-v3 и enforcement-приёмку: на fresh `synchronize` ещё нужны exact current parents и полный aggregate PASS, а на реальной защите — доказательство, что candidate duplicate не удовлетворяет правило вместо trusted run, отмена analysis не разрешает merge, изменение/удаление candidate workflow не выключает проверку и отсутствие независимого control-owner approval блокирует merge. Merge-group HEAD отдельно проверяется только в topology с доступной merge queue. До этих проверок T3 не получает `VERIFIED`.
