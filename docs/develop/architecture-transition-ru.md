...
Дата обновления: 2026-09-13. Статус: T0 реализован для снимка из [отчёта инвентаризации](t0-provenance-report-ru.md); T1 зафиксирован как `BASELINE_COMPLETE_WITH_KNOWN_FAILURES`; T2 принят как `REPORT_ONLY_COMPLETE_WITH_CLASSIFIED_LIMITS`, а не как архитектурный/dead-code/build PASS. T3 control PR #4, #12 и #31 последовательно слиты в `main`; PR #31 интегрирован exact merge-коммитом `7843651969329f34accf537e902fa77c7720a4ff`. Первый post-integration `opened` PR #32 доказал исправление merge-ref race: run `34777290557` при пустом payload без retrigger принял synthetic merge `7d24b626030517461d81d45de3292474f64b6bf3` с точными base/head parents. Тот же run затем выявил отдельный trusted fixture-harness regression: шесть source-mutation cases не нашли protected resolver dependency, поэтому analysis дал `6 failed, 112 passed, 118 subtests passed`, а final остался fail-closed. PR #32 закрыт без merge. Исправление `2eba9b2c69204da713a26b5acaaa96c6d7f64512` материализует полный exact protected fixture closure; focused Windows subset дал `130 passed, 1 skipped`, полный non-root Linux quality-suite — `483 passed` без skip. Оно ещё не интегрировано и требует отдельного PR/review/разрешения, затем полного повторения post-integration матрицы. Репозиторий принадлежит личному аккаунту, Required Workflow и merge queue недоступны в этой topology, ruleset отсутствует, независимый control owner не назначен. Поэтому T3 остаётся `IN_PROGRESS / REPORT_ONLY / NOT_ENABLED`, а T4–T7 по этой последовательности не приняты.
