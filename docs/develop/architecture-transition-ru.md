...
Дата обновления: 2026-09-13. Статус: T0 реализован для снимка из [отчёта инвентаризации](t0-provenance-report-ru.md); T1 зафиксирован как `BASELINE_COMPLETE_WITH_KNOWN_FAILURES`; T2 принят как `REPORT_ONLY_COMPLETE_WITH_CLASSIFIED_LIMITS`, а не как архитектурный/dead-code/build PASS. T3 control PR #4, #12 и #31 последовательно слиты в `main`; PR #31 интегрирован exact merge-коммитом `7843651969329f34accf537e902fa77c7720a4ff`. Первый post-integration `opened` PR #32 доказал исправление merge-ref race: run `34777290557` при пустом payload без retrigger принял synthetic merge `7d24b626030517461d81d45de3292474f64b6bf3` с точными base/head parents. Тот же run затем выявил отдельный trusted fixture-harness regression: шесть source-mutation cases не нашли protected resolver dependency, поэтому analysis дал `6 failed, 112 passed, 118 subtests passed`, а final остался fail-closed. PR #32 закрыт без merge. Candidate PR #33 содержит исправление `2eba9b2c69204da713a26b5acaaa96c6d7f64512`, которое материализует полный exact protected fixture closure, и follow-up `4ae9c53a0`, отключающий встроенный pytest cache provider: без него успешные fixtures создавали новый `.pytest_cache`, а post-capture обязан был отклонять snapshot как `INCOMPLETE`. Windows subset завершился `131 passed, 1 skipped`; целевой non-root Linux fixture subset — `132 passed`, полный Linux quality-suite — `484 passed`, оба без skip. Исправления ещё не интегрированы; после интеграции требуется полное повторение post-integration матрицы. Репозиторий принадлежит личному аккаунту, Required Workflow и merge queue недоступны в этой topology, ruleset отсутствует, независимый control owner не назначен. Поэтому T3 остаётся `IN_PROGRESS / REPORT_ONLY / NOT_ENABLED`, а T4–T7 по этой последовательности не приняты.
...
**Текущий результат.** Реализованы exact changed-path selector, base/candidate monotonic comparison, protected-source materialization, identity-bound reports и fail-closed aggregate. Candidate checker/workflow остаются `MUST_MATCH`; только declarative policy может пройти `BASE_FIXTURE_REVIEW`, не меняя base evaluator/materialization. Workflow разделяет trusted plan, candidate analysis и fresh final aggregate с единственным стабильным именем `architecture-policy`. Configured Python producer запускается в Docker с read-only candidate/trusted/rootfs, без сети/IPC и только с одним supervisor-owned report file; runtime children имеют UID/GID `65534:65534`, пустые supplementary groups, нулевые effective capabilities и `no_new_privileges`. Перед producer обязательны 12 отрицательных OS checks. Исторические positive/negative runs перечислены в [отчёте T3](t3-architecture-policy-ru.md). PR #4/#12/#31 интегрированы в trusted `main`; PR #31 merge `784365196…` добавил stable exact `read/fetch/read`, fetched object/parent/payload binding, verified plan checkout и cross-job receipt. Первый реальный `opened` после интеграции, PR #32 run [34777290557](https://github.com/meownm/ragflow/actions/runs/34777290557), успешно разрешил пустой payload в exact merge `7d24b626…` с правильными родителями — primary resolver race исправлена. Полный run не прошёл: base-owned fixture harness материализовал только test files без protected resolver dependency, получил `6 failed, 112 passed, 118 subtests passed`, и final корректно отказал. Отдельное исправление `2eba9b2c69204da713a26b5acaaa96c6d7f64512` требует exact `source_bundle == protected_sources`, использует общий digest-checking materializer и добавляет positive/negative closure contract. Windows focused subset завершился `130 passed, 1 skipped`; полный Linux suite в pinned image, Python 3.13.11, UID/GID `1000:1000`, read-only filesystem, без network/IPC/capabilities и с NNP завершился `483 passed` без skip. Исправление ещё не интегрировано; deploy и repository settings не менялись. Полный контракт и доказательства находятся в разделе [«Нормативный контракт PR merge-ref resolver»](t3-architecture-policy-ru.md#нормативный-контракт-pr-merge-ref-resolver). Результат остаётся `IN_PROGRESS / NOT_ENABLED`; `CLASSIFIED`, `PLANNED`, `OBSERVED` и `NOT_EVALUATED` не становятся `PASS`.
