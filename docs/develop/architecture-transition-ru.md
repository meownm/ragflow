...
Дата обновления: 2026-09-12. Статус: T0 реализован для снимка из [отчёта инвентаризации](t0-provenance-report-ru.md); T1 зафиксирован как `BASELINE_COMPLETE_WITH_KNOWN_FAILURES`; T2 принят как `REPORT_ONLY_COMPLETE_WITH_CLASSIFIED_LIMITS`, а не как архитектурный/dead-code/build PASS. Для T3 отревьюирован control update, а candidate `d6025f428dbdf0420bc14228f440a7df1b91f5ef` прошёл два положительных, пять отрицательных и одну exact-tip base-mode GitHub-пробу. Последующая проверка реального enforcement выявила незакрытые control-path дефекты: отсутствовал `merge_group`, применялся `cancel-in-progress`, а Actions использовали mutable tags. Инкремент `b8f7772fc165d37fba01b23b93fc869046be8767` заменяет PR-trigger на base-owned `pull_request_target`, добавляет merge-queue trigger, immutable candidate SHA и SHA-pinned Actions; `51d6a28ae1c71f495cc666cd9abbaf56e9f07c16` дополнительно выбирает trusted fixtures при изменении любого workflow и отклоняет duplicate/dynamic `architecture-policy` context. Оба изменения ещё не проверены после интеграции в default branch. Репозиторий принадлежит личному аккаунту, Required Workflow доступен только на уровне organization/enterprise ruleset, независимый control owner отсутствует. Поэтому T3 остаётся `IN_PROGRESS / REPORT_ONLY / NOT_ENABLED`, а T4–T7 по этой последовательности не приняты.
...
**Текущий результат.** Реализованы exact changed-path selector, base/candidate monotonic comparison, protected-source materialization, identity-bound reports и fail-closed aggregate. Candidate checker/workflow остаются `MUST_MATCH`; только declarative policy может пройти `BASE_FIXTURE_REVIEW`, не меняя base evaluator/materialization. Workflow разделяет trusted plan, candidate analysis и fresh final aggregate с единственным стабильным именем `architecture-policy`. Configured Python producer запускается в Docker с read-only candidate/trusted/rootfs, без сети/IPC и только с одним supervisor-owned report file; runtime children имеют UID/GID `65534:65534`, пустые supplementary groups, нулевые effective capabilities и `no_new_privileges`. Перед producer обязательны 12 отрицательных OS checks. Base-mode runs [34691682512](https://github.com/meownm/ragflow/actions/runs/34691682512) и [34692464102](https://github.com/meownm/ragflow/actions/runs/34692464102) прошли, а runs `34692465518`, `34692466560`, `34692467687`, `34692469616` и `34692471425` доказали fail-closed для сужения policy, отсутствующего analyzer, failed trusted fixture, запрещённой зависимости и unclassified core path; exact-tip run `34693499337` подтвердил commit `d6025f428dbdf0420bc14228f440a7df1b91f5ef`. Эти runs не покрывают последующий enforcement-hardening `b8f7772fc165d37fba01b23b93fc869046be8767`. В нём workflow загружается из trusted default branch через `pull_request_target`, отдельно обрабатывает `merge_group/checks_requested`, не отменяет выполняющийся required run, связывает все job с одним immutable candidate SHA, не сохраняет checkout credentials и закрепляет все Actions полными commit SHA. Результат остаётся `IN_PROGRESS / NOT_ENABLED`: `CLASSIFIED`, `PLANNED`, `OBSERVED` и `NOT_EVALUATED` не становятся `PASS`, а новый trigger нельзя считать принятым до реальной post-integration пробы.

**Действия.** Не создавать пустой baseline вместо недоказанного широкого verdict. Сохранить единственный финальный job с устойчивым именем `architecture-policy`. После отдельного разрешения интегрировать control candidate в trusted default branch. Для personal-account repository организация-level Required Workflow недоступен, поэтому до изменения правил нужно явно выбрать один из доказуемых вариантов: перенести репозиторий в организацию и закрепить Required Workflow либо использовать repository ruleset вместе с проверенным base-owned `pull_request_target`/внешним GitHub App check. Обычный одноимённый status от GitHub Actions не принимается как эквивалент без негативной пробы против candidate duplicate/spoof. Требуется добавить независимого владельца control paths: единственный текущий collaborator и автор PR не может одобрить собственный PR. Gate запускается независимо от метки `ci`; path filters не должны оставлять required job навсегда pending.

**Приёмка.** Исторические PR-пробы для `d6025f428dbdf0420bc14228f440a7df1b91f5ef` выполнены: разрешённый candidate прошёл, запрещённая dependency и unclassified core path заблокированы, отсутствие analyzer и failed fixture не превратились в skip, а configured Python runtime code не получил доступа к supervisor-owned evidence/command files в проверенной OS boundary. Они не закрывают изменённый workflow `b8f7772fc165d37fba01b23b93fc869046be8767` и duplicate-context fixture `51d6a28ae1c71f495cc666cd9abbaf56e9f07c16`. После их попадания в trusted default branch нужно доказать на реальном repository enforcement, что `pull_request_target` публикует требуемый check именно для актуального PR/merge-group SHA, candidate duplicate не удовлетворяет правило вместо trusted run, отмена analysis не разрешает merge, изменение/удаление candidate workflow не выключает проверку и отсутствие независимого control-owner approval блокирует merge. До этих проверок T3 не получает `VERIFIED`.
