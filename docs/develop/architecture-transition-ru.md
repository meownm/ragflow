...
Дата обновления: 2026-09-14. Статус: T0 реализован для снимка из [отчёта инвентаризации](t0-provenance-report-ru.md); T1 зафиксирован как `BASELINE_COMPLETE_WITH_KNOWN_FAILURES`; T2 принят как `REPORT_ONLY_COMPLETE_WITH_CLASSIFIED_LIMITS`, а не как архитектурный/dead-code/build PASS. T3 control PR #4, #12, #31 и #33 последовательно слиты в `main`; PR #33 интегрирован обычным merge без bypass exact commit `312c9ce8cd92163e42350537ecc906112f850017` из head `ea34a7e1a7ea9cec395f0fb8837051b68a880b56`. Disposable PR #34 на этой trusted base завершил функциональную post-integration матрицу: первичный `opened`, `ready_for_review`, `reopened`, одиночный и быстрые последовательные `synchronize`; полный plan/analysis/final, resolver receipts, 123 attested fixtures, 12 OS negative controls и check-run API binding. Rapid intermediate head `b405926346c7dcd6aff0af00a1e2f59da365f8b3` fail-closed остановлен run `34810676603`, когда live ref уже соответствовал следующему head; финальный head `6fac9d826281ba777412b6d3c26c88b8812669e0` принят run `34810680171`, а browser run `34810681642` прошёл Chromium, Firefox, WebKit и aggregate gate. PR #34 закрыт без merge. Это закрывает функциональную live-приёмку, но не enforcement: успешные aggregate по-прежнему `REPORT_ONLY_COMPLETE / NOT_ENABLED`, `main` не защищена, ruleset отсутствует, repository принадлежит личному аккаунту, независимый control owner не назначен и duplicate-context/approval/control-removal probes ещё не выполнялись под реальной защитой. Поэтому T3 остаётся `IN_PROGRESS / REPORT_ONLY / NOT_ENABLED`, а T4–T7 по этой последовательности не приняты.
