...
Дата обновления: 2026-09-14. Статус: T0 реализован для снимка из [отчёта инвентаризации](t0-provenance-report-ru.md); T1 зафиксирован как `BASELINE_COMPLETE_WITH_KNOWN_FAILURES`; T2 принят как `REPORT_ONLY_COMPLETE_WITH_CLASSIFIED_LIMITS`, а не как архитектурный/dead-code/build PASS. T3 control PR #4, #12, #31, #33, #36 и #37 последовательно слиты в `main`. PR #37 интегрирован обычным merge без bypass exact commit `30a8123fb1f483934dc094d6cef2135ac125a3c6` с родителями `[bd50920559170dbc9b042b26168062d1de777b36, 30eab490a5ded077c8928908b3580e795467d767]`; resolver schema v3 теперь берёт authoritative default-branch base из `github.sha`, требует `github.workflow_sha == github.sha` и хранит `pull_request.base.sha` только как advisory-наблюдение. Первый post-integration `edited` run PR #35 `34834573688` подтвердил эту identity-часть и ожидаемо отказался принимать stale merge ref: 42 bounded-наблюдения, затем `PARENT_MISMATCH`; candidate не был выпущен. Следующий fresh `synchronize` run [34835069719](https://github.com/meownm/ragflow/actions/runs/34835069719) принял exact synthetic merge `1481df4482601ceb65cf39b551782e53d752e7b6` с родителями `[30a8123fb1f483934dc094d6cef2135ac125a3c6, 3bb7993374f625e2d5e4c4516969bc120c9af49a]`: plan, analysis и canonical final завершились успешно; identity и selection byte-identical на всех трёх стадиях, compatibility — в plan/final. Protected 18-source fixture bundle дал `PASS`, 70/70 required nodeids и 131 cases без failure/error/skip. Browser run [34835073348](https://github.com/meownm/ragflow/actions/runs/34835073348) завершил frontend, Chromium, Firefox, WebKit и browser-gate; каждый движок выполнил 52 сценария на том же merge и manifest. Это закрывает функциональную schema-v3 re-acceptance, но не enforcement: aggregate остаётся `REPORT_ONLY_COMPLETE / NOT_ENABLED`, `main` не защищена, ruleset отсутствует, repository принадлежит личному аккаунту, независимый control owner не назначен и duplicate-context/approval/control-removal probes ещё не выполнялись под реальной защитой. Поэтому T3 остаётся `IN_PROGRESS / REPORT_ONLY / NOT_ENABLED`, а T4–T7 по этой последовательности не приняты.
...
**Текущий результат.** Реализованы exact changed-path selector, base/candidate monotonic comparison, protected-source materialization, identity-bound reports и fail-closed aggregate. Candidate checker/workflow остаются `MUST_MATCH`; только declarative policy может пройти `BASE_FIXTURE_REVIEW`, не меняя base evaluator/materialization. Workflow разделяет trusted plan, candidate analysis и fresh final aggregate с единственным стабильным именем `architecture-policy`. Configured Python producer запускается в Docker с read-only candidate/trusted/rootfs, без сети/IPC и только с одним supervisor-owned report file; runtime children имеют UID/GID `65534:65534`, пустые supplementary groups, нулевые effective capabilities и `no_new_privileges`. Перед producer обязательны 12 отрицательных OS checks. PR #33 с resolver receipt v2 и fixture-harness fixes интегрирован в trusted `main` merge `312c9ce8cd92163e42350537ecc906112f850017`; disposable PR #34 затем завершил v2 event/identity matrix. PR #36 замкнул protected fixture bundle и слит merge `bd50920559170dbc9b042b26168062d1de777b36`. Последующий run PR #35 `34818108224` выявил default-branch advancement: `github.sha=bd509205…`, но payload base сохранил `312c9ce8…`; stable merge имел правильных родителей `[bd509205…, 7a1e0f04…]`, а v2 resolver ошибочно продолжал считать payload base authoritative и завершился `PARENT_MISMATCH`. PR #37 исправил этот контракт, прошёл full protected Linux suite `143 passed` без skip и browser gate на трёх движках, после отдельного разрешения слит обычным merge `30a8123fb1f483934dc094d6cef2135ac125a3c6`. Его push run [34834327053](https://github.com/meownm/ragflow/actions/runs/34834327053) ожидаемо не самоутвердил изменённые `MUST_MATCH` sources относительно первого родителя и не засчитан как acceptance. Первый schema-v3 `edited` run PR #35 [34834573688](https://github.com/meownm/ragflow/actions/runs/34834573688) принял current default-branch base как authority и stale payload base как advisory, но отказался принимать не обновившийся merge ref: 42 `STALE`, затем bounded `PARENT_MISMATCH`. Fresh `synchronize` run [34835069719](https://github.com/meownm/ragflow/actions/runs/34835069719) принял current exact-parent merge `1481df4482601ceb65cf39b551782e53d752e7b6`; plan, analysis и final успешны, base compatibility и protected policy-fixtures — `PASS`, 70/70 required nodeids и 131 cases. Связанный browser run [34835073348](https://github.com/meownm/ragflow/actions/runs/34835073348) дал общий gate `PASS` для Chromium/Firefox/WebKit, по 52 сценария на одном manifest. Функциональная schema-v3 re-acceptance завершена. Deploy и repository settings не менялись. Результат остаётся `IN_PROGRESS / REPORT_ONLY / NOT_ENABLED`: отсутствие активной защиты и независимого владельца не позволяет считать T3 `VERIFIED`; `CLASSIFIED`, `PLANNED`, `OBSERVED` и `NOT_EVALUATED` не становятся `PASS`.

**Действия.** Не создавать пустой baseline вместо недоказанного широкого verdict и сохранить единственный финальный job с устойчивым именем `architecture-policy`. Следующий шаг требует внешних решений: выбрать доказуемую topology enforcement, указать независимого trusted reviewer/control owner и только после отдельного разрешения включить защиту без bypass. Для personal-account repository organization-level Required Workflow недоступен; варианты — перенос в организацию с подходящим планом и Required Workflow/merge queue либо personal ownership с repository ruleset и отдельно проверенным base-owned `pull_request_target`/pinned external GitHub App check. После включения защиты выполнить live negative probes против duplicate green context, отменённого analysis, удаления/переименования workflow, изменения control path, stale head и отсутствующего approval; каждый сценарий должен быть немержабельным. Gate запускается независимо от метки `ci`; path filters не должны оставлять required job навсегда pending.

**Приёмка.** Функциональная post-integration часть DoD для receipt v2 выполнена на PR #34, а schema-v3 re-acceptance после default-branch advancement — на PR #35. Первичный `opened` при пустом payload прошёл с первой delivery; `ready_for_review`, `reopened`, одиночный и rapid `synchronize` сохранили exact identity semantics. Финальный v2 artifact аттестует `123` fixture cases без failure/error/skip; в них выполнены controlled stale (5), changing (4), missing/disappearing (5) и non-commit/wrong-parent (5) варианты. Live rapid-проба дополнительно не выпустила candidate для устаревшего промежуточного head. Schema-v3 run `34834573688` доказал, что stale payload base не заменяет authoritative `github.sha`, а stale merge ref не выпускает candidate. Последующий run `34835069719` принял exact parents `[30a8123f…, 3bb79933…]`; plan/analysis/final используют byte-identical identity receipt `df8f095d…` и selection `7b5957ff…`, plan/final — byte-identical compatibility `5b026a48…`. Canonical check `architecture-policy` единственный для exact head и создан GitHub Actions app `15368`. Fixture report source — base-owned 18-file bundle: `PASS`, 70/70 required nodeids, 131 cases, 251 JUnit test elements, ноль failure/error/skip; aggregate — `REPORT_ONLY_COMPLETE / NOT_ENABLED`. Browser receipts связали Chromium, Firefox и WebKit с тем же merge и manifest `d2321a9d…`, по 52 сценария. Сканирование architecture и browser logs этого delivery не нашло незамаскированных GitHub token/Authorization/Actions runtime token/AWS access-key patterns. Это завершает функциональную schema-v3 часть, но не enforcement-приёмку: на реальной защите ещё нужно доказать, что candidate duplicate не удовлетворяет правило вместо trusted run, отмена analysis не разрешает merge, изменение/удаление candidate workflow не выключает проверку и отсутствие независимого control-owner approval блокирует merge. Merge-group HEAD отдельно проверяется только в topology с доступной merge queue. До этих проверок T3 не получает `VERIFIED`.
