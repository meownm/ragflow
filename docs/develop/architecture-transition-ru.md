...
Дата обновления: 2026-09-12. Статус: T0 реализован для снимка из [отчёта инвентаризации](t0-provenance-report-ru.md); T1 зафиксирован как `BASELINE_COMPLETE_WITH_KNOWN_FAILURES`; T2 принят как `REPORT_ONLY_COMPLETE_WITH_CLASSIFIED_LIMITS`, а не как архитектурный/dead-code/build PASS. Для T3 [base-policy protocol](t3-architecture-policy-ru.md) находится в `origin/main`: реальные base-mode GitHub запуски подтвердили authoritative plan, fixture-only positive path и fail-closed блокировку изменённых protected sources. Полный четырёхполосный прогон выявил Linux-несовместимость ASR venv launcher; текущая версия 0.8.1 разрешает только symlink с точной конечной целью `sys.executable`, отклоняет произвольную внешнюю цель и локально проходит весь quality-suite. Повторный полный base-mode прогон, OS-level runtime isolation, enforced control-owner approval, branch protection и Required Workflow ruleset ещё не подтверждены. Поэтому T3 сохраняет `REPORT_ONLY / NOT_ENABLED`; T3–T7 не приняты.
...
**Текущий результат.** Реализованы exact changed-path selector, fail-closed aggregate и сравнение base/candidate coverage в `check_architecture_policy.py` с политикой `architecture-policy.json`. Selector учитывает committed/staged/unstaged/untracked изменения, обе стороны rename/delete через `--no-renames`, текущую T0-классификацию и владельца удалённого пути из базовой `module-map.yaml`. Plan закрепляет source bundle и ожидаемые producer/config hashes; aggregate требует отдельный identity-bound `policy-compatibility.json` и отклоняет суженный план, stale identity, подмену protected source, самодекларированный producer без правильных digest и неполное evidence. Protected producers материализуются из authority bundle и запускаются через `python -I`; pytest contracts отключают ambient conftest и очищают child environment, а их прямой test-harness closure закреплён policy. При `authority_source=base` ordinary protected analyzer/config/fixture/dependency sources обязаны совпасть с base; только declarative candidate policy допускает изменение с отметкой `BASE_FIXTURE_REVIEW`, базовой materialization, монотонным policy comparison и обязательным ручным review. Checker и workflow остаются `MUST_MATCH`; их обновление, protected fixture updates и сужение controls требуют отдельного аудированного protocol/bootstrap flow. Bootstrap использует candidate source bundle и подтверждает только self-consistency. Analysis до lifecycle setup отклоняет весь запуск, если выбранный lane не имеет intact source bundle. Fixture runner выполняет exact trusted `path::nodeid` между двумя T0 captures и является `success()`-предусловием остальных producers; Node parser ставится во временной копии materialized authority manifest/lock/config без scripts, pnpm hooks и workspace, а в candidate tree переносится только dereferenced TypeScript package. Только в реальном base-mode эти package-manager inputs закреплены base; bootstrap использует candidate self-check bundle и не является supply-chain границей. Runner передаёт candidate root через явный environment contract и встраивает проверяемый JUnit в JSON-attestation. Workflow `.github/workflows/architecture.yml` запускается без метки `ci` и path filters и разделён на trusted plan, candidate analysis и fresh final aggregate. Только финальный job имеет устойчивый display name `architecture-policy`; он повторно checkout-ит candidate tree, заново извлекает checker/capture/policy из comparison base, повторяет selection/base-candidate comparison, сверяет compatibility с plan artifact, загружает отдельный analysis artifact и не запускает candidate dependency install, analyzers или pytest. Тесты фиксируют три job, artifact handoff, обязательный compatibility, isolated producer запуск, отсутствие candidate lifecycle в plan/final и новое извлечение authority перед aggregate. Protocol v1 теперь находится в comparison base; режим всё равно остаётся `REPORT_ONLY_COMPLETE / NOT_ENABLED`, а `CLASSIFIED` и `PLANNED` не становятся `PASS`. Fixture-only base-mode run `34681312892` прошёл; полный policy-expansion run `34681858657` корректно остановился на Linux launcher incompatibility. Исправление 0.8.1 проходит 380 тестов quality-suite и ожидает повторного полного base-mode запуска. Отрицательные GitHub-пробы, OS isolation, enforced control-owner approval и Required Workflow ruleset остаются обязательными до приёмки T3.

**Действия.** Разобрать существующий собственный долг, создать baseline без новых нарушений, сохранить единственный финальный job с устойчивым именем `architecture-policy` и закрепить workflow из trusted default branch через Required Workflow ruleset либо эквивалентный внешний pinned check. Он запускается независимо от метки `ci`; path filters не должны оставлять required job навсегда pending.

**Приёмка.** Проверочный PR с запрещённой зависимостью и PR с неучтённым изменением ядра блокируются. Разрешённый PR проходит. Проверка отсутствия данных/инструмента завершается ошибкой, а не успешным skip. Candidate runtime code не получает доступа к supervisor-owned evidence/command files в проверенной OS boundary. После отмены job merge не становится разрешённым автоматически; изменение candidate workflow не заменяет pinned Required Workflow. До проверки runtime isolation и repository ruleset этап не завершён.
