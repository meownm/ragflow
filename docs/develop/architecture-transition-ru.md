...
Дата обновления: 2026-09-14. Статус: T0 реализован для снимка из [отчёта инвентаризации](t0-provenance-report-ru.md); T1 зафиксирован как `BASELINE_COMPLETE_WITH_KNOWN_FAILURES`; T2 принят как `REPORT_ONLY_COMPLETE_WITH_CLASSIFIED_LIMITS`, а не как архитектурный/dead-code/build PASS. T3 control PR #4, #12, #31, #33, #36 и #37 последовательно интегрированы в `main`; текущая trusted base — merge-коммит `30a8123fb1f483934dc094d6cef2135ac125a3c6`. Functional post-integration acceptance receipt schema v3 получен на PR #35: stale-ref run `34834573688` завершился fail-closed, а fresh runs `34835069719`, `34836369666` и `34837242457` подтвердили exact head/base identity, protected fixtures и canonical final `success`. PR #35 остаётся draft и не должен быть слит. Для personal repository включён active no-bypass ruleset `main-solo-maintainer-gate-v1` (`23306934`): только PR в `main`, запрет deletion/non-fast-forward, strict required `architecture-policy`, привязанный к GitHub Actions App `15368`; approvals и code-owner review равны нулю. Диагностические PR #38/#39 закрыты без merge и доказали positive path, блокировку нового head после отмены analysis, блокировку удаления workflow, user-created status spoof, позднего same-name/same-App green и `skipped` duplicate. Настоящий инкремент добавляет `CODEOWNERS` для `@meownm` и нормативный solo-maintainer профиль. До его обычной интеграции и live readback T3 остаётся `IN_PROGRESS`; целевой solo-вердикт — `VERIFIED_WITH_ACCEPTED_RISK`, а не `VERIFIED`. T4–T7 по последовательному плану до этого не начинаются.
...
T2 включает T0 provenance, ограниченные Python ARC-01/02/03 и DEAD-01 scan/plan, 19 exact runtime probes/contracts, полный `web/src` TypeScript observer, два Linux Go package graphs, runtime-graph classifier и Go build-profile planner. TypeScript computed import остаётся исходным `INCOMPLETE`; Go observer и свежий build plan остаются `OBSERVED / NOT_EVALUATED`; classifier возвращает только `COMPLETE / REPORT_ONLY / REVIEW_REQUIRED`; DEAD-01 сохраняет `NOT_CONFIRMED`. T3 выполняет выбранные lanes в base-authority workflow и изолирует configured Python runtime probes на уровне Docker. Внешний ruleset теперь блокирует merge по canonical check, но машинный aggregate намеренно сохраняет `REPORT_ONLY_COMPLETE / NOT_ENABLED`: он не может сам аттестовать настройки GitHub. Широкий архитектурный/dead-code/native-build verdict, исторические saved DSL и продуктовые regression checks этим не заменяются.
...
**Текущий результат.** Реализованы exact changed-path selector, base/candidate monotonic comparison, protected-source materialization, identity-bound reports и fail-closed aggregate. Candidate checker/workflow остаются `MUST_MATCH`; только declarative policy может пройти `BASE_FIXTURE_REVIEW`, не меняя base evaluator/materialization. Workflow разделяет trusted plan, candidate analysis и fresh final aggregate с единственным стабильным именем `architecture-policy`. PR #33, #36 и #37 закрыли fixture closure, base-tip authority и receipt schema v3; trusted `main` закреплён на `30a8123fb1f483934dc094d6cef2135ac125a3c6`. PR #35 дал функциональную post-integration приёмку schema v3, но остаётся draft и не входит в `main`. Active repository ruleset `23306934` без bypass требует strict `architecture-policy` от GitHub Actions App `15368`, PR flow и запрещает deletion/non-fast-forward. Protected disposable PR #38 подтвердил разрешённый path, stale-success invalidation и fail-closed cancellation; PR #39 подтвердил control-removal, user-status, same-name/same-App и skipped-duplicate negatives. Оба закрыты без merge. Машинный aggregate по-прежнему честно сообщает `REPORT_ONLY_COMPLETE / NOT_ENABLED`; внешний enforcement подтверждается отдельным live receipt. До интеграции `CODEOWNERS`/receipt-инкремента результат `IN_PROGRESS`; затем он может быть принят только как `VERIFIED_WITH_ACCEPTED_RISK`. `CLASSIFIED`, `PLANNED`, `OBSERVED`, `NOT_EVALUATED`, `INCOMPLETE` и `FAIL` не становятся успехом.

**Действия.** Зарегистрировать `.github/CODEOWNERS` как extension `quality-governance`, включить его в policy-fixture selector/protected sources и обновить T0 snapshot. Провести focused local tests, затем обычный protected PR без admin/bypass. После его интеграции повторно прочитать exact main SHA, CODEOWNERS, ruleset, required check App binding и отсутствие bypass. Не создавать пустой baseline вместо недоказанного широкого verdict. PR #35 не сливать; deploy не выполнять.

**Приёмка.** `VERIFIED_WITH_ACCEPTED_RISK` допускается только для явно выбранного solo-maintainer профиля и только когда автоматические применимые lanes успешны/доказуемо `NOT_APPLICABLE`, canonical check привязан к exact head и App, ruleset активен без bypass, положительная и все доступные отрицательные enforcement-пробы дали ожидаемый mergeability verdict, CODEOWNERS находится в protected `main`, а принятые риски и условия пересмотра записаны. Текущие принятые риски: нет независимого reviewer; sole admin может изменить ruleset; personal-account topology не предоставляет organization Required Workflow и merge queue; GitHub сопоставляет status по context/App, а не по immutable workflow identity. Эти риски не превращают `FAIL`, `INCOMPLETE`, `OBSERVED` или `NOT_EVALUATED` в PASS и не позволяют использовать статус `VERIFIED`. Стандартный `VERIFIED` по-прежнему требует независимого control-owner approval и topology с неизменяемым workflow/queue-equivalent enforcement.
...
```text
Этап: T0..T7
Статус: PLANNED / IN_PROGRESS / VERIFIED / VERIFIED_WITH_ACCEPTED_RISK / INCOMPLETE
...
`VERIFIED` требует всех критериев соответствующего этапа. `VERIFIED_WITH_ACCEPTED_RISK` — отдельный, более слабый терминальный статус только для заранее определённого профиля: обязательные автоматические проверки и доступные enforcement-пробы должны быть выполнены, а недостижимые в выбранной topology гарантии перечислены как конкретные принятые риски с владельцем и условиями пересмотра. Он не повышает `FAIL`, `INCOMPLETE`, `OBSERVED`, `NOT_EVALUATED`, `PLANNED` или `CLASSIFIED` до успеха и не может сокращаться до `VERIFIED` в отчётах.

Для последовательности этапов `VERIFIED_WITH_ACCEPTED_RISK` закрывает зависимость только в явно принятом пользователем профиле и только в его зафиксированных границах; следующие этапы наследуют этот risk receipt. Любое расширение write/admin-состава, смена владельца, ruleset, required context/App, trusted workflow, доступность organization enforcement или merge queue требует повторной T3-приёмки. Проект документа, существование YAML или отсутствие замечаний без проверки не заменяют доказательства.
