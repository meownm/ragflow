...
Дата обновления: 2026-09-12. Статус: T0 реализован для снимка из [отчёта инвентаризации](t0-provenance-report-ru.md); T1 зафиксирован как `BASELINE_COMPLETE_WITH_KNOWN_FAILURES`; T2 принят как `REPORT_ONLY_COMPLETE_WITH_CLASSIFIED_LIMITS`, а не как архитектурный/dead-code/build PASS. Для T3 отревьюирован control update и подготовлен candidate `c130751d6965aa558612c4085b39189c441bbc20`: два положительных и пять отрицательных base-mode GitHub runs подтвердили full-lane execution, fail-closed controls и Docker OS-boundary configured Python runtime probes. Техническая приёмка candidate завершена, но Required Workflow ruleset, защита default branch и обязательное control-owner approval не настроены и не проверены. Поэтому T3 остаётся `IN_PROGRESS / REPORT_ONLY / NOT_ENABLED`, а T4–T7 по этой последовательности не приняты.
