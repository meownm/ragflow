...
Дата обновления: 2026-09-14. Статус: T0 реализован для снимка из [отчёта инвентаризации](t0-provenance-report-ru.md); T1 зафиксирован как `BASELINE_COMPLETE_WITH_KNOWN_FAILURES`; T2 принят как `REPORT_ONLY_COMPLETE_WITH_CLASSIFIED_LIMITS`, а не как архитектурный/dead-code/build PASS. T3 control PR #4, #12, #31, #33 и #36 последовательно слиты в `main`; PR #36 интегрирован обычным merge без bypass exact commit `bd50920559170dbc9b042b26168062d1de777b36` с родителями `[312c9ce8cd92163e42350537ecc906112f850017, c88adc7aababd4b42262033167c6ea504a53c0c0]`. Он замкнул base-owned `policy-fixtures` bundle двумя hash-locked requirements-файлами: Ready run `34816852206` полностью прошёл plan/analysis/final, материализовал 18 protected sources и аттестовал 123 cases без failure/error/skip; browser run `34815293154` прошёл Chromium, Firefox, WebKit и aggregate gate. Ранее disposable PR #34 на trusted base `312c9ce8…` завершил функциональную event/identity матрицу, включая fail-closed rapid stale-head run `34810676603` и успешный финальный run `34810680171`, после чего был закрыт без merge. Первый post-merge `edited` run PR #35 `34817327341` не является приёмкой новой базы: GitHub сохранил старый synthetic merge `f1c549136d442b6fccd5284e2b852c5a5463dedc` с родителями `[312c9ce8…, 92e6e3a5…]`, plan материализовал прежние 16 sources, analysis не нашёл requirements-файл и завершился exit 2, а canonical final остался красным. Это отрицательное fail-closed доказательство; свежий `synchronize` ещё обязан подтвердить exact base `bd509205…`. Функциональные доказательства не закрывают enforcement: успешные aggregate по-прежнему `REPORT_ONLY_COMPLETE / NOT_ENABLED`, `main` не защищена, ruleset отсутствует, repository принадлежит личному аккаунту, независимый control owner не назначен и duplicate-context/approval/control-removal probes ещё не выполнялись под реальной защитой. Поэтому T3 остаётся `IN_PROGRESS / REPORT_ONLY / NOT_ENABLED`, а T4–T7 по этой последовательности не приняты.
...
**Текущий результат.** Реализованы exact changed-path selector, base/candidate monotonic comparison, protected-source materialization, identity-bound reports и fail-closed aggregate. Candidate checker/workflow остаются `MUST_MATCH`; только declarative policy может пройти `BASE_FIXTURE_REVIEW`, не меняя base evaluator/materialization. Workflow разделяет trusted plan, candidate analysis и fresh final aggregate с единственным стабильным именем `architecture-policy`. Configured Python producer запускается в Docker с read-only candidate/trusted/rootfs, без сети/IPC и только с одним supervisor-owned report file; runtime children имеют UID/GID `65534:65534`, пустые supplementary groups, нулевые effective capabilities и `no_new_privileges`. Перед producer обязательны 12 отрицательных OS checks. PR #33 с resolver receipt v2 и fixture-harness fixes интегрирован в trusted `main` merge `312c9ce8cd92163e42350537ecc906112f850017`. Затем disposable PR #34 без повторного trigger успешно прошёл `opened` run [34808357865](https://github.com/meownm/ragflow/actions/runs/34808357865), `ready_for_review` [34808915668](https://github.com/meownm/ragflow/actions/runs/34808915668), `reopened` [34809190659](https://github.com/meownm/ragflow/actions/runs/34809190659), одиночный `synchronize` [34809939910](https://github.com/meownm/ragflow/actions/runs/34809939910) и финальный rapid-sync [34810680171](https://github.com/meownm/ragflow/actions/runs/34810680171). Промежуточный rapid head ожидаемо fail-closed остановлен [34810676603](https://github.com/meownm/ragflow/actions/runs/34810676603): 26 наблюдений `STALE`, затем bounded `TIMEOUT`, analysis не запускался, canonical final был `failure`. Для финального SHA plan/analysis/final получили byte-identical identity receipt; API показал ровно один canonical `architecture-policy` от app `15368/github-actions` на каждом из промежуточного и финального SHA с выводами `failure` и `success` соответственно. PR #34 закрыт без merge. Отдельный fixture-closure PR #36 на head `c88adc7aababd4b42262033167c6ea504a53c0c0` добавил в base policy два пропущенных protected requirements-файла, прошёл Ready run [34816852206](https://github.com/meownm/ragflow/actions/runs/34816852206) и после явного разрешения слит обычным merge `bd50920559170dbc9b042b26168062d1de777b36`. Первый `edited` run PR #35 после merge [34817327341](https://github.com/meownm/ragflow/actions/runs/34817327341) сохранил прежнюю event base и old synthetic merge, поэтому корректно упал на отсутствии requirements; он не засчитывается вместо нового-base `synchronize`. Deploy и repository settings не менялись. Функциональная event/identity матрица завершена, но результат остаётся `IN_PROGRESS / REPORT_ONLY / NOT_ENABLED`: отсутствие активной защиты и независимого владельца не позволяет считать T3 `VERIFIED`; `CLASSIFIED`, `PLANNED`, `OBSERVED` и `NOT_EVALUATED` не становятся `PASS`.
...
**Приёмка.** Функциональная post-integration часть DoD выполнена на PR #34. Первичный `opened` при пустом payload прошёл с первой delivery; `ready_for_review`, `reopened`, одиночный и rapid `synchronize` сохранили exact identity semantics. Финальный artifact аттестует `123` fixture cases без failure/error/skip; в них выполнены controlled stale (5), changing (4), missing/disappearing (5) и non-commit/wrong-parent (5) варианты. Live rapid-проба дополнительно не выпустила candidate для устаревшего промежуточного head. Во всех успешных deliveries plan/analysis/final используют byte-identical receipt и один base/head/candidate tuple; check-run API связал canonical context с exact head, workflow, event и GitHub Actions app. Сканирование шести architecture run logs не нашло незамаскированных GitHub token/Authorization/Actions runtime token/AWS access-key patterns. Это не заменяет оставшуюся enforcement-приёмку: на реальной защите ещё нужно доказать, что candidate duplicate не удовлетворяет правило вместо trusted run, отмена analysis не разрешает merge, изменение/удаление candidate workflow не выключает проверку и отсутствие независимого control-owner approval блокирует merge. Merge-group HEAD отдельно проверяется только в topology с доступной merge queue. До этих проверок T3 не получает `VERIFIED`.
