...
| E3 | Изолированные browser-golden lifecycle, schema workspace, ручной SQL и LLM-assisted SQL, включённые в обязательный реестр из 56 cases | **DONE, 4 сценария, 3/3 браузера** | E1, E2, E4–E6 |
...
| E5 | Decision workflow: варианты при неоднозначности, явный выбор таблиц/полей, обязательное подтверждение каждого JOIN/WHERE и общий no-SQL gate | **DONE для текущего constructor slice**; долговременный audit aggregate отсутствует | E4 |
