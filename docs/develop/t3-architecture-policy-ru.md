...
- штатный `capture_inventory.py --write` выполнен без network/supporting refresh; повторный read-only capture для head `35283256f97bc2d22548a0156ec3dbd5df490a3e` содержит 751 запись, `unstaged_delta=8` после обновления двух self-recording registry files и `unclassified=0`.
...
1. Опубликовать локально готовое исправление stateful resolver в текущем Ready PR #31, сверить pushed SHA с проверенным diff и подтвердить ожидаемый fail-closed результат текущего `MUST_MATCH` base gate. Интеграция требует отдельного явного разрешения. Только после появления resolver в trusted `main` на новом disposable PR доказать, что первое реальное событие `opened` получает точный synthetic merge commit и проходит без ручного `edited` retrigger, после чего выполнить всю post-integration матрицу выше.
