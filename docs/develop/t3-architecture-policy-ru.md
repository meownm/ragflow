...
Дата: 2026-09-12. Статус: `REPORT_ONLY_BASE_PROTOCOL_EXERCISED`. Protocol v1 вошёл в `origin/main` через PR #1, merge commit `db7d6cfc22b74853790b9f5fd21a55756cc56e86`; исправление сохранения `.venv` launcher symlink и browser-regression receipt вошло через PR #3, merge commit `e9e5c608bf78fbcaa1afcbba2ba4eafb59510df2`. Bootstrap fixture runner после этого выполнил 22/22 обязательных проверок. Последующие GitHub base-mode запуски подтвердили authoritative plan и совместимость политики, а analysis штатно закрылся как `INCOMPLETE`, поскольку тот же кандидат менял защищённые `pyproject.toml`, `module-map.yaml`, checker и fixture sources. Текущий control update устраняет ложное расхождение LF Git blobs и CRLF Windows checkout для неизменённых sources/fixtures, но отклоняет любое иное скрытое расхождение Git-чистого source; его новый bootstrap fixture contract локально проходит 24/24. Положительная base-mode приёмка после включения этого control update в comparison base ещё не выполнена. Required Workflow ruleset и branch protection отсутствуют; OS-level isolation намеренно враждебного candidate runtime не реализован.
...
Использовать новый пустой evidence directory; tool откажется перезаписывать старые результаты. Сначала явно разрешить comparison base текущего PR. Если base содержит protocol v1, checker и policy необходимо извлечь именно из неё; `candidate-bootstrap` допустим только для исторической базы без protocol и проверяет лишь self-consistency кандидата. Перед запуском убедиться, что локальный `origin/main` соответствует фактической PR base, либо подставить её точный commit SHA.
...
Отдельный запуск checker/policy из текущего `origin/main` вернул `authority_source=base`, `compatibility_status=COMPATIBLE`, но ожидаемо `source_integrity=INCOMPLETE` для всех четырёх выбранных lane: версия 0.3.1 сравнивает LF Git blobs с CRLF checkout bytes. GitHub run `34559216165` ранее подтвердил ту же fail-closed последовательность для иного protected-source кандидата: plan прошёл, analysis остановился до lifecycle setup, fresh final отказал из-за незавершённого analysis. На момент ревью API возвращает пустой список repository rulesets, branch `main` не защищена, `sha_pinning_required=false`; OS-level isolation candidate runtime и enforced control-owner approval отсутствуют. Поэтому control update готов только к отдельному PR/bootstrap review, а T3 остаётся `NOT_ENABLED`.

## Что осталось до приёмки T3

1. Protocol v1 уже зафиксирован в base. Реальными PR подтвердить четыре base-mode сценария: безвредное монотонное расширение policy получает `BASE_FIXTURE_REVIEW`, сохраняет base materialization и `COMPATIBLE`; сужение policy отклоняется; изменение checker/workflow или другого ordinary protected source даёт `INCOMPLETE`; провал trusted fixture не может дать успешный aggregate. Evidence должен сообщать `evaluator_source=base` и `evaluated_policy_source=base`.
