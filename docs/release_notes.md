...
- Data sources: Adds private-Dataset connectors for EVA Wiki and OpenMetadata with incremental sync, pruning, secret-preserving edits, and structured catalog metadata.
- OpenMetadata: Adds an authenticated Catalog Copilot, six managed Canvas Agent Apps, semantic Dataset retrieval, lineage and quality views, and preview/confirm governance.
- Agent runtime: Adds beta-token file uploads for embedded agents and routes uploaded video through Speech-to-Text.
- Sandbox: Adds a durable local gVisor runtime bootstrap for self-managed CodeExec pools.

### Improvements and bug fixes

- Enforces Dataset and domain visibility across OpenMetadata catalog and relationship routes.
- Makes connector failures retryable without advancing sync cursors and keeps PostgreSQL sync-log queries portable.
- Gates nginx startup on backend readiness and keeps the Python API restart loop observable.
- Completes frontend type checks and production-build compatibility for the local integration surface.
