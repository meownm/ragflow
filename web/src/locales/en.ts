export default {
  translation: {
    ...
    openMetadata: {
      ...
      subtitle:
        'Search metadata, owners and domains, inspect the OpenMetadata knowledge graph, and make governed changes from Агент Раггер.',
...
Example: A 1 KB message with 1024-dim embedding uses ~9 KB. The 5 MB default limit holds ~500 such messages.`,
    ...
    knowledgeDetails: {
      ...
      retrievalTestingDescription:
        'Conduct a retrieval test to check if Агент Раггер can recover the intended content for the LLM.',
      Parse: 'Parse',
      dataset: 'Dataset',
      ...
      rerankTip: `Optional. If left empty, Агент Раггер will use a combination of weighted keyword similarity and weighted vector cosine similarity; if a rerank model is selected, a weighted reranking score will replace the weighted vector cosine similarity. Please be aware that using a rerank model will significantly increase the system's response time. If you wish to use a rerank model, ensure you use a SaaS reranker; if you prefer a locally deployed rerank model, ensure you start Агент Раггер with docker-compose-gpu.yml.`,
      topK: 'Top-K',
...
`,
    ...
    knowledgeConfiguration: {
      ...
      tableColumnModeAutoDescription:
        'All columns are included in chunk text and stored as metadata (Агент Раггер default).',
...
**Essential Rules:**
  ...
  - **Always consider** the entire conversation history.`,
      ...
      **Essential Rules:**
      ...
      betaError:
        'Please acquire a Агент Раггер API key from the System Settings page first.',
      searching: 'Searching...',
      parsing: 'Parsing',
      uploading: 'Uploading',
...
Example: Virtual Hosted Style`,
      ...
      evaTokenSaveFailed: 'Failed to save EVA token.',
      evaTokenDeleteFailed: 'Failed to delete EVA token.',
      ...
      embeddingModelTip:
        'The default embedding model for each newly created dataset. If you cannot find an embedding model from the dropdown, check if you are using Агент Раггер slim edition (which does not include embedding models) or check https://ragflow.io/docs/dev/supported_models to see if your model provider supports this model.',
      img2txtModel: 'VLM',
      img2txtModelTip:
...
[Insert text here]`,
    ...
    admin: {
      ...
      auditPage: {
        title: 'Audit',
        description:
          'User and background process events from the last {{days}} days.',
        refresh: 'Refresh',
        events: 'events',
        failures: 'failures',
        activeSources: 'active sources',
        searchPlaceholder: 'Action, user, object, or chain ID',
        ...
        emptyHint: 'Change the filters or search query.',
        sources: {
      ...
      navigationVisibilityPage: {
        description:
          'Globally choose which sections appear in the main menu for all users.',
        visibleCount: 'Visible: {{count}} of {{total}}',
        showAll: 'Show all',
        hideAll: 'Hide all',
        save: 'Save',
        saving: 'Saving...',
        saved: 'Section visibility updated.',
      },
