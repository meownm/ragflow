export default {
  translation: {
    ...
    openMetadata: {
      ...
      subtitle:
        'Поиск метаданных, владельцев и доменов, анализ графа знаний OpenMetadata и управляемые изменения внутри Агент Раггер.',
...
Пример: сообщение 1 КБ с эмбеддингом 1024 измерений — около 9 КБ. Лимит по умолчанию 5 МБ — примерно 500 таких сообщений.`,
    ...
    knowledgeDetails: {
      ...
      retrievalTestingDescription:
        'Проведите тест извлечения, чтобы проверить, может ли Агент Раггер восстановить целевой контент для LLM.',
      Parse: 'Парсинг',
      dataset: 'Датасет',
      ...
      rerankTip: `Опционально. Если оставить пустым, Агент Раггер будет использовать комбинацию взвешенной схожести по ключевым словам и взвешенного косинусного сходства векторов; если выбрана rerank модель, взвешенный reranking счет заменит взвешенное косинусное сходство векторов. Имейте в виду, что использование rerank модели значительно увеличит время отклика системы. Если вы хотите использовать rerank модель, убедитесь, что вы используете SaaS reranker; если вы предпочитаете локально развернутую rerank модель, убедитесь, что вы запускаете Агент Раггер с помощью docker-compose-gpu.yml.`,
      topK: 'Топ-K',
...
**Основные правила:**
  ...
  - **Всегда учитывайте** всю историю диалога.`,
      ...
      betaError:
        'Пожалуйста, сначала получите Агент Раггер API ключ на странице Системных настроек.',
      searching: 'Поиск...',
      parsing: 'Парсинг',
      uploading: 'Загрузка',
...
Пример: Virtual Hosted Style`,
      ...
      evaApiBaseUrl: 'Базовый URL API EVA',
      evaApiBaseUrlRequired: 'Необходимо указать базовый URL API EVA',
      evaApiBaseUrlTip:
        'URL, доступный с сервера Агент Раггер. Для локального сервиса Docker используйте URL шлюза Docker-хоста.',
      evaWebBaseUrl: 'Базовый веб-URL EVA',
      evaWebBaseUrlTip:
        'Публичный URL для браузера, используемый в ссылках на источники. По умолчанию используется базовый URL API.',
      evaConnectorApiToken: 'Токен API EVA',
      evaConnectorTokenPlaceholder:
        'Обязателен при создании; оставьте пустым, чтобы сохранить текущий токен',
      evaConnectorTokenRequired: 'Необходимо указать токен API EVA',
      evaProject: 'Проект EVA',
      evaProjectRequired: 'Необходимо выбрать проект EVA',
      evaProjectTip:
        'Выберите проект, доступный указанному токену EVA. После создания коннектора проект изменить нельзя. Коннектор EVA можно связать только с приватной базой знаний Агент Раггер.',
      evaIncludeAttachments: 'Загружать вложения',
      evaVerifySslCertificate: 'Проверять SSL-сертификат',
      evaIncludeArchivedPages: 'Загружать архивные страницы',
      evaBatchSize: 'Размер пакета',
      evaBatchSizeValidation: 'Размер пакета должен быть от 1 до 1000',
      evaAttachmentSizeLimit: 'Лимит размера вложения (байт)',
      evaAttachmentSizeValidation:
        'Лимит вложения должен быть от 1 байта до 100 МиБ',
      evaPageSizeLimit: 'Лимит размера страницы (байт)',
      evaPageSizeValidation: 'Лимит страницы должен быть от 1 байта до 100 МиБ',
      evaRetryCount: 'Количество повторных попыток',
      evaRetryCountValidation:
        'Количество повторных попыток должно быть от 0 до 10',
      evaEnterApiAndTokenFirst: 'Сначала укажите URL API EVA и токен.',
      evaProjectsLoadError: 'Не удалось загрузить проекты EVA.',
      evaProjectsNotFound: 'Доступные проекты EVA не найдены.',
      evaConfiguredProjectInaccessible:
        'Настроенный проект EVA больше недоступен.',
      evaProjectSelectPlaceholder: 'Выберите проект EVA',
      evaProjectLoadPlaceholder: 'Загрузите доступные проекты',
      evaProjectsEmpty: 'Нет доступных проектов EVA',
      evaProjectsReload: 'Обновить проекты',
      evaProjectsLoad: 'Загрузить проекты',
      ...
      evaTokenNoConnectors:
        'Нет доступных подключений EVA. Настройте соединение в админке раздела «Документы» или в источниках данных.',
      ...
      evaTokenUsageDescription:
        'В управлении документами сначала используется сервисный токен EVA-коннектора, а при его отсутствии — ваш персональный токен. Персональный токен также используется для сохранения черновика и публикации; фоновая синхронизация по-прежнему требует сервисный токен.',
      ...
      evaTokenDeleteConfirmation:
        'После удаления чтение EVA без сервисного токена коннектора, сохранение черновика и публикация изменений будут недоступны, пока вы не добавите персональный токен снова.',
      ...
      embeddingModelTip:
        'Модель эмбеддингов по умолчанию для каждой новой созданной базы знаний. Если вы не можете найти модель эмбеддингов в выпадающем списке, проверьте, используете ли вы облегченную версию Агент Раггер (которая не включает модели эмбеддингов) или проверьте https://ragflow.io/docs/dev/supported_models, чтобы узнать, поддерживает ли ваш провайдер моделей эту модель.',
      img2txtModel: 'VLM',
      img2txtModelTip:
...
[Вставьте текст здесь]`,
    ...
    admin: {
      ...
      navigationVisibility: 'Видимость разделов',
      accessGroups: 'Группы доступа',
      accessGroupsPage: {
        new: 'Новая группа',
        create: 'Создание группы',
        edit: 'Настройка группы',
        name: 'Название',
        description: 'Описание',
        users: 'Пользователи',
      ...
      auditPage: {
        title: 'Аудит',
        description:
          'События пользователей и фоновых процессов за последние {{days}} дней.',
        refresh: 'Обновить',
        events: 'событий',
        failures: 'ошибок',
        activeSources: 'активных источников',
        searchPlaceholder: 'Действие, пользователь, объект или ID цепочки',
        allSources: 'Все источники',
        allOutcomes: 'Все результаты',
        unavailableSources: 'Недоступны источники: {{sources}}',
        time: 'Время',
        outcome: 'Результат',
        event: 'Событие',
        actor: 'Пользователь',
        ...
        source: 'Источник',
        context: 'Контекст',
        correlationId: 'ID цепочки',
        causationId: 'Причина',
        requestId: 'ID запроса',
        traceId: 'ID трассы',
        spanId: 'ID спана',
        interactionId: 'ID действия',
        jobId: 'ID задачи',
        sessionId: 'ID сессии',
        errorId: 'ID ошибки',
        openTrace: 'Открыть трассу',
        openLogs: 'Открыть логи',
        error: 'Ошибка',
        details: 'Технические детали',
        chain: 'Цепочка событий',
        empty: 'События не найдены',
        emptyHint: 'Измените фильтры или поисковый запрос.',
        sources: {
          application: 'Приложение',
          business_documents: 'Документы',
      ...
      navigationVisibilityPage: {
        description:
          'Глобально выберите разделы, которые отображаются в основном меню для всех пользователей.',
        visibleCount: 'Показано: {{count}} из {{total}}',
        showAll: 'Показать все',
        hideAll: 'Скрыть все',
        save: 'Сохранить',
        saving: 'Сохранение...',
        saved: 'Видимость разделов обновлена.',
      },
      ...
      businessDocumentsSettingsPage: {
        ...
        tokenSaved: 'Токен сохранён. Оставьте пустым, чтобы сохранить его.',
        tokenOptional: 'Необязательно',
        tokenHelp:
          'Общий токен используется для чтения и загрузки списка пространств. Без него документы читаются с личным токеном из профиля. Запись в EVA всегда выполняется с личным токеном.',
        clearToken: 'Удалить сохранённый общий токен',
        evaSpace: 'Пространство EVA',
        evaSpaceHelp:
          'Введите ID пространства или загрузите список с общим токеном. Поиск и изменения документов ограничены этим пространством. При смене API или пространства привяжите документы заново.',
        loadSpaces: 'Загрузить пространства',
        loadingSpaces: 'Загрузка…',
        evaSpacesEmpty: 'Нет доступных пространств',
        verify_ssl: 'Проверять SSL-сертификат',
        include_archived: 'Включать архивные страницы и пространства',
        disconnect: 'Отключить соединение',
        save: 'Сохранить',
        saving: 'Сохранение…',
        saved: 'Соединение EVA для раздела «Документы» обновлено.',
      },
